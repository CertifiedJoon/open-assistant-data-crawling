...
class WordLocator:
  def __init__(self, url : str):
    ...
    self._crawled = set(url[:-1] for url in open(f"./result/txt_located.txt", "r",  encoding="utf-8").readlines())
  ...
  def crawl_for_word(self, start, word):
    ...
    f_located = open(f"./result/txt_located.txt", "a",  encoding="utf-8")
  ...
  def crawl(self, start):
    ...
    sheet = self._wb.active
    sheet.append(['Instruction', 'Response', 'Source', 'MetaData'])
    filename = f"./result/{datetime.datetime.today().strftime('%Y-%m-%d')}__crawled_data.xlsx"
    f_located = open(f"./result/txt_located.txt", "a",  encoding="utf-8")
